...
@section('title', __('backend_meetings.labels.management') . ' | ' . __('backend_meetings.labels.create'))
...
@section('content')
    {{ html()->form('POST', route('admin.meetings.store'))->class('form-horizontal')->open() }}
    {{-- Select options ($users, $hosts, $statuses) are provided by the controller --}}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">@lang('backend_meetings.labels.create')</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
                ...
                <div class="col">
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            ...
                            {{ html()->hidden('request_number', $nextRequestNumber ?? '') }}
                        </div><!--col-->
                    </div><!--form-group-->
                    <div class="form-group row mt-3">
                        {{ html()->label('Requested By')->class('col-md-2 form-control-label')->for('requested_by') }}
                        <div class="col-md-4">
                            {{ html()->select('requested_by', $users, auth()->id())->class('form-control select2')->attribute('style','width:100%') }}
                        </div>

                        {{ html()->label('Date Requested')->class('col-md-2 form-control-label')->for('date_requested') }}
                        <div class="col-md-4">
                            {{ html()->date('date_requested')->class('form-control')->value($defaultDateRequested ?? '') }}
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-3">
                            {{ html()->date('date_scheduled')->class('form-control')->value($defaultDateScheduled ?? '') }}
                        </div>

                        {{ html()->label('Start Time')->class('col-md-2 form-control-label')->for('time_start') }}
                        <div class="col-md-2">
                            {{ html()->time('time_start')->class('form-control')->value($defaultTimeStart ?? '') }}
                        </div>

                        {{ html()->label('End Time')->class('col-md-1 form-control-label')->for('time_end') }}
                        <div class="col-md-2">
                            {{ html()->time('time_end')->class('form-control')->value($defaultTimeEnd ?? '') }}
                        </div>
                    </div>
                    <hr>
                    <div class="form-group row">
                        ...
                        <div class="col-md-4">
                            {{ html()->select('statuses_id', $statuses, 1)->class('form-control') }}
                        </div>
                    </div>
...
<script>
    (function($){
        $(function(){
            ...
            var $start = $('input[name="time_start"]');
            var $end = $('input[name="time_end"]');
